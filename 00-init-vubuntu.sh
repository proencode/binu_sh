...
${cGreen}----> (7) ${cBlue}/bin/sh${cReset} -> ${cRed}bash${cYellow} 로 되어있지 않으므로 수정합니다. 다음 작업에서 '''아니오 (No)''' 를 선택해 주세요.
...
__EOF__
	...
	echo "----> (8) echo \"dash dash/sh boolean false\" | sudo debconf-set-selections ; DEBIAN_FRONTEND=noninteractive ; sudo dpkg-reconfigure dash"
	echo "dash dash/sh boolean false" | sudo debconf-set-selections
	DEBIAN_FRONTEND=noninteractive
	sudo dpkg-reconfigure dash
	#xxx--- cmdRun "sudo dpkg-reconfigure dash" "#---> '''아니오 (No)''' 를 선택해 주세요."
	cmdRun "sudo ls -al --color /bin/sh"
	echo "${cGreen}----> (9) ${cCyan}bash 로 수정했습니다. 스크립트를 다시 실행하세요.${cReset}"
	exit 1
fi
#----
#----
#----
cmdYenter "sudo apt -y update ; sudo apt-get -y update" "(10) 시스템 업데이트"
cmdYenter "sudo apt -y upgrade ; sudo apt-get -y upgrade" "(11) 시스템 업그레이드"
cmdRun "sudo apt -y install gcc g++ make perl git build-essential p7zip-full p7zip-rar vim net-tools  openssh-server xrdp gnome-tweaks" "(12) 기본으로 설치할 프로그램들"
cmdRun "dpkg -l | grep kernel" "(13) kernel 버전 확인"
#----
#----
#----
cmdYenter "sudo /sbin/rcvboxadd quicksetup all" "(14) 이작업 시작전에  '''장치 > 게스트 확장 CD 이미지 삽입 > sudo ./VBoxLinuxAdditions.run > 오류시 재작업'''  을 먼저 끝내야 합니다."
cmdYenter "grep vboxsf /etc/group" "(15) vboxsf 그룹 확인"
cmdYenter "sudo gpasswd -a ${USER} vboxsf ; grep vboxsf /etc/group" "(16) 사용자를 vboxsf 그룹에 추가합니다."
...
cmdRun "sudo apt-get install vim" "(17) vim 설치"
cmdRun "git clone https://github.com/VundleVim/Vundle.vim.git ~/.vim/bundle/Vundle.vim" "(18) VundleVim 설치"
cmdRun "cp DOTvimrc-vubuntu ~/.vimrc" "(19) .vimrc 설치"
echo "${cGreen}----> (20) ${cYellow}vi +BundleInstall +qall Bundle 설치${cReset}"
...
cmdRun "sudo apt-get install -y libsecret-1-0 libsecret-1-dev" "(21) 라이브러리 설치"
cmdRun "cd /usr/share/doc/git/contrib/credential/libsecret ; sudo make" "(22) 컴파일"
cmdRun "git config --global credential.helper /usr/share/doc/git/contrib/credential/libsecret/git-credential-libsecret" "(23) 설치"
cat <<__EOF__
#-- github 비번관리
...
__EOF__
cmdYenter "git config credential.helper store" "(24) 이와 같이 저장합니다."
