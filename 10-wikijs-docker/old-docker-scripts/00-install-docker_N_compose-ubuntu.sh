...
cat_and_run "sudo apt -y update && sudo apt -y upgrade" "업데이트 및 업그레이드"
...
echo "${cGreen}----> ${cYellow}sudo add-apt-repository \"deb [arch=amd64] https://download.docker.com/linux/ubuntu $(lsb_release -cs) stable\"${cReset}"
